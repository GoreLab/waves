---
title: "Introduction to the `waves` package"
author: Jenna Hershberger [jmh579@cornell.edu]
date: "`r Sys.Date()`"
...
## 1. Format your data
Match spectra with reference values so that you have a dataframe with unique identifiers, reference values, and other metadata as columns to the left of spectral values. Spectral column names should start with "X". Remove rows with missing values.
```{r}
ikeogu.2017[1:7,1:7]
...
ikeogu.2017.prepped[1:5,1:5]
```


## 2. Visualize spectra using `plot_spectra()` 
...
## 3. Filter spectra to remove outliers with `filter_spectra()`.
...
filtered.df[1:5, c(1:5, (ncol(filtered.df) - 5):ncol(filtered.df))]
```
...
## 4. If you have more than one scan per unique identifier, aggregate the scans by mean or median with `aggregate_spectra()`.
...
## 5. Use `test_spectra()` to perform spectral pretreatment, cross-validation set formation, and model training functions over multiple iterations.
...
See `?test_spectra()` for more information on the arguments and output for this function. A description of output is below under section [5.4](#5.4. Output).
...
### 5.1. Specify which spectral pretreatments (1-13) to apply with the parameter `pretreatment`.
`pretreat_spectra()` can also be used on its own to transform a data.frame using any/all of 12 available pretreatments:
...
ikeogu.2017.prepped[1:10,] %>% # subset the first 10 scans for speed
  ...
  mutate(wl = as.numeric(stringr::str_extract(.data$wl, "\\-*\\d+\\.*\\d*")),
...
model.to.save$best.model.stats %>% 
  ...
  pivot_wider(id_cols = c(Pretreatment, summary_type), names_from = statistic, values_from = value)
```
...
cbind(unique.id = pretreated.val$unique.id, 
  ...
  coord_fixed(xlim = overall.range,
                  ylim = overall.range) + 
  labs(title = "Example dry matter content prediction from ikeogu.2017",
       x = "Observed",
       y = "Predicted") + 
  theme_bw()
```
